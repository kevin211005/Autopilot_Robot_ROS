...
class ParticleFilter():
  ...
  def initialize_global(self):
    ...
    height, width = np.where(self.permissible_region == 1)
    idx = np.random.choice((len(height)), self.N_PARTICLES)
    poses = np.zeros(self.particles.shape)
    poses[:, 0], poses[:, 1], poses[:, 2] = width[idx], height[idx], 2 * np.pi * np.random.random(self.N_PARTICLES)

    Utils.map_to_world(poses, self.map_info)
  ...
  def expected_pose(self):
    # YOUR CODE HERE
    x, y, theta = self.particles[:, 0], self.particles[:, 1], self.particles[:, 2]
    weights = self.weights[:]
    avg_x, avg_y = np.sum(x * weights), np.sum(y * weights)
    avg_theta = np.arctan2(np.sum(np.sin(theta) * weights), np.sum(np.cos(theta)*weights))
  ...
  def clicked_pose_cb(self, msg):
    ...
    # YOUR CODE HERE
    msg_pose_x, msg_pose_y, msg_pose_theta = msg.pose.pose.position.x, msg.pose.pose.position.y, Utils.quaternion_to_angle(msg.pose.pose.orientation)
    print("Initialpose changed: x = ", msg_pose_x, "y = ", msg_pose_y, "orientation", msg.pose.pose.orientation)
    map_service_name = rospy.get_param("~static_map", "static_map")
    print("Getting map from service: ", map_service_name)
    rospy.wait_for_service(map_service_name)
    map_info = rospy.ServiceProxy(map_service_name, GetMap)().map.info
    print(ut.world_to_map([msg_pose_x, msg_pose_y],map_info))
    self.particles[:, 0] = msg_pose_x + np.random.normal(0, 0.5, self.N_PARTICLES)
    self.particles[:, 1] = msg_pose_y + np.random.normal(0, 0.5, self.N_PARTICLES)
    self.particles[:, 2] = msg_pose_theta + np.random.normal(0, 0.5, self.N_PARTICLES)
    self.weights[:] = 1 / float(self.N_PARTICLES)
