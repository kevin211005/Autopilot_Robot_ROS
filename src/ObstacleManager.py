...
class ObstacleManager(object):
	...
	def get_state_validity(self, config):
		...
		length = int(math.sqrt((self.robotWidth ** 2 + self.robotLength ** 2) / 2))
		l, r = mapConfig[0] - length, mapConfig[0] + length
		d, u = mapConfig[1] - length, mapConfig[1] + length
		###Out of bound 
		if l < 0 or r >= self.mapWidth or d < 0 or  u >= self.mapHeight:
			return False 
		### obstacle in car occupied area 
		if self.mapImageBW[d:u, l:r].sum() != 0:
			return False 
		return True
	...
	def discretize_edge(self, config1, config2):
		config1, config2 = numpy.array(config1[:2]), numpy.array(config2[:2])
		edgeLength = numpy.linalg.norm(config2 - config1)
		N = int(edgeLength / self.collision_delta) + 1 
		intermediate_point = [list(numpy.linspace(config1[i], config2[i], N)) for i in range(2)]
		list_x, list_y = intermediate_point[0], intermediate_point[1]
