...
KM_V_NOISE = 0.0125# Kinematic car velocity noise std dev
KM_DELTA_NOISE = 0.125# Kinematic car delta noise std dev
KM_X_FIX_NOISE = 0.0125# Kinematic car x position constant noise std dev
KM_Y_FIX_NOISE = 0.0125# Kinematic car y position constant noise std dev
...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
    ...
    curr_angle =(self.last_servo_cmd - self.STEERING_TO_SERVO_OFFSET) / self.SPEED_TO_ERPM_GAIN
    ...
    ## generate normal distrution: random.normal(loc=0.0, scale=1.0, size=None)
    curr_speed += np.random.normal(0, KM_V_NOISE, num_particles)
    curr_angle += np.random.normal(0, KM_DELTA_NOISE, num_particles)
    ## Beta = arctan(1/2 * tan(Theta))
    beta = np.arctan(0.5*np.tan(curr_angle))
    delta_T = msg.header.stamp.secs - self.last_vesc_stamp.secs
    #/#
    #particles[0]  = xt 
    #particles[1]  = yt
    #particles[2] = theta
    ...
    for i in range(num_particles):
      theta_t1 = self.particles[i,2]
      #theta t 
      self.particles[i, 2] += curr_speed[i] * delta_T * np.sin(2 * beta[i]) / self.CAR_LENGTH
      # xt 
      self.particles[i,0] +=  self.CAR_LENGTH * (np.sin(self.particles[i, 2]) - np.sin(theta_t1)) / np.sin(2 * beta[i])
      # yt
      self.particles[i,1] +=  self.CAR_LENGTH * (-np.cos(self.particles[i, 2]) + np.cos(theta_t1)) / np.sin(2 * beta[i])
    
    # Sample model noise for each particle
    self.particles[:, 0] += np.random.normal(0, KM_X_FIX_NOISE, num_particles)
    self.particles[:, 1] += np.random.normal(0, KM_Y_FIX_NOISE, num_particles)
    self.particles[:, 2] += np.random.normal(0, KM_THETA_FIX_NOISE, num_particles)
