...
Z_MAX =  0.62# Weight for max reading
Z_RAND =  0.1# Weight for random reading
Z_HIT = 0.27# Weight for hit reading
...
class SensorModel:
  ...
  def lidar_cb(self, msg):
    ...
    # print("angle", msg.angle_max - msg.angle_min)
    ##down_sample 
    n = int(len(num) / self.LASER_RAY_STEP)
    obs_arr = np.zeros((2, n), dtype = np.float32)
    for i in range(n):
      range_measure = num[i*self.LASER_RAY_STEP]
      obs_arr[0,i] = range_measure if range_measure and not math.isnan(range_measure) else self.MAX_RANGE_METERS
      obs_arr[1,i] = msg.angle_min + i*self.LASER_RAY_STEP * msg.angle_increment 
    obs = (obs_arr[0,:], obs_arr[1,:])
    # print("obs", np.shape(obs[0]))
    # print(obs)
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    for d in range(table_width):
      p_hit_sum = np.zeros((table_width,))
      for r in range(table_width):
        ##p = z_hit*p_hit + z_short*p_short + z_max*p_max + z_rand *p_rand 
        ## cal p_hit 
        # if 0 <= r <= Z_MAX:
        #   N = (1.0/ np.sqrt(2*np.pi*(SIGMA_HIT**2))) * np.exp(-0.5 * ((r-d)/ SIGMA_HIT)**2)
        #   p_hit_sum[r] = N
        # else:
        #   N = 0
        N = (1.0/ np.sqrt(2.0*np.pi*(SIGMA_HIT**2))) * np.exp(-0.5 * ((r-d)/ SIGMA_HIT)**2)
        p_hit_sum[r] = N
        ##Cal p_short 
        if 0 <= r <= d and d != 0:
          p_short = 1.0 / (1 - np.exp(-LAMBDA_SHORT * d)) * (LAMBDA_SHORT*np.exp(-LAMBDA_SHORT*r))
        else:
          p_short = 0 

        ##cal p_max 
        p_max = 1.0 if r == max_range_px else 0
        
        ##cal p_rand 
        p_rand = 1.0 / max_range_px if 0 <= r <= max_range_px else 0 
        sensor_model_table[r, d] = Z_SHORT * p_short + Z_MAX * p_max + Z_RAND * p_rand
      sensor_model_table[:, d] += Z_HIT * p_hit_sum / np.sum(p_hit_sum)
  ...
  def apply_sensor_model(self, proposal_dist, obs, weights):
    ...
    # print("weight_in_function", weights)
