...
class PlannerNode(object):
  ...
  def get_plan_cb(self, req):
    ...
    print(req.source)
    source_in = req.source
    target_in = req.target 
    pose_source, pose_target =  Pose(), Pose()
    pose_source.position.x = source_in[0]
    pose_source.position.y = source_in[1]
    pose_source.orientation = Utils.angle_to_quaternion(source_in[2])

    pose_target.position.x = target_in[0]
    pose_target.position.y = target_in[1]
    pose_target.orientation = Utils.angle_to_quaternion(target_in[2])
    req.source = pose_source
    req.target = pose_target 
    self.source_pose = [req.source.position.x,
                        req.source.position.y]
    self.source_yaw = Utils.quaternion_to_angle(req.source.orientation)
    self.source_updated = True
    self.source_lock.release()    
    
    self.target_lock.acquire()
    self.target_pose = [req.target.position.x,
                        req.target.position.y]
    self.target_yaw = Utils.quaternion_to_angle(req.target.orientation)
  ...
  def update_plan(self):
    ...
    if replan:
      ...
      # print(source_pose[0], source_pose[1])
      # point = Utils.world_to_map(source_pose, self.map_info) + [0.0]
      # print(point)
      # print(Utils.map_to_world(point, self.map_info))
      # print(Utils.world_to_map(Utils.map_to_world(point, self.map_info), self.map_info))
